...
public class Player implements IPlayer {
    ...
    public Player(CardToPlayStrategy cardToPlayStrategy, SelectColorStrategy selectColorStrategy, String nombre) {
    ...
    public boolean needsToDrawCard(ICard currentCard) {
        for (ICard card : cartas) {
            if (card.isPlayableOver(currentCard)) return false;
        }
        return true;
    }
    ...
    public ICard getCardFromHand(int number) {
        if(number>0 && number<cartas.size()) return cartas.get(number);
        return new NullCard();
    }
