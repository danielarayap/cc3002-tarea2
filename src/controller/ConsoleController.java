...
public class ConsoleController implements IController {
     ...
     *
     * @param game that controls the game logic
     * @param view that controls the console view
     */
    ...
    public int AskForCardFromHand(IPlayer player) {
        ...
        while (num < 0 || num > player.getHandSize()) {
            ...
            num = in.nextInt();
            if (num == player.getHandSize()) {
                view.showMessage("[Estado]: " + game.getCurrentPlayer() + " roba una carta");
                game.drawOneCard(player);
            }
        }
        return num;
    }
